...
const IngredientCategoryManagement: React.FC = (props: any) => {
  const {
    params,
    ingredientCategoryList,
    fetchIngredientCategoryList,
  ...
  const handelDelete = (id: string) => {
    const payload = { id };
    deleteIngredientCategory(payload);
  };

  const suffix = (
    <Tooltip placement="top" title={'Search ingredient category'}>
      <SearchOutlined
        style={{
          fontSize: 16,
          color: '#1890ff',
  ...
  const showConfirm = (values: any) => {
    confirm({
      maskClosable: true,
      title: 'Do you want to delete this ingredient category ?',
      icon: (
    ...
    confirm({
      ...
      onOk: (close) => {
        return updateFrom.validateFields().then(async (items) => {
          ...
          await updateIngredientCategory(payload);
    ...
    confirm({
      ...
      onOk: (close) => {
        return createFrom.validateFields().then(async (items) => {
          ...
          await createIngredientCategory(payload);
  ...
  return (
    <Spin spinning={loadingFetchIngredientCategoryList}>
      <BasicLayout>
          ...
          <div className={`${styles.container}`}>
            <div className={`${styles.title}`}>
              <Row justify="space-between">
                <Col>Ingredient Category Management</Col>
                <Col>
                  <Row gutter={[8, 8]}>
                    <Col>
                      <Input
                    ...
                    <Col>
                      <Button type="primary" onClick={handleClickCreate}>
                        <Tooltip placement="top" title={'Click to create'}>
                          <PlusOutlined /> New ingredient category
                        </Tooltip>
                      </Button>
                    </Col>
                  </Row>
            ...
            <div className={`${styles.table}`}>
              <Table
                ...
                columns={columns}
                bordered
                dataSource={ingredientCategoryList}
...
const mapDispatchToProps = (dispatch: any) => ({
  fetchIngredientCategoryList: (payload: any) =>
    dispatch({ type: 'ingredientCategory/fetchIngredientCategoryList', payload }),
  deleteIngredientCategory: (payload: any) =>
    dispatch({ type: 'ingredientCategory/deleteIngredientCategory', payload }),
  updateIngredientCategory: (payload: any) =>
    dispatch({ type: 'ingredientCategory/updateIngredientCategory', payload }),
  createIngredientCategory: (payload: any) =>
    dispatch({ type: 'ingredientCategory/createIngredientCategory', payload }),
  saveParams: (payload: any) => dispatch({ type: 'ingredientCategory/saveParams', payload }),
  refresh: () => dispatch({ type: 'ingredientCategory/refresh' }),
});

export default connect(mapStateToProps, mapDispatchToProps)(IngredientCategoryManagement);
